...
public class InputStreamScene extends Scene {
    ...
    public InputStreamScene(InputStream inputStream) throws IOException {
        ...
        //如果直接不解析分辨率decodeStream的话可以正常返回Bitmap对象，这个原因很简单，第一次decodeStream时已经操作过inputstream
        // 了，这时候流的操作位置已经移动了，如果再次decodeStream则不是从流的起始位置解析，所以无法解析出Bitmap对象。
        inputStream.reset();
...
//        Rect rect = new Rect(0,0,-1080,1860);
        Bitmap bitmap = null;
        if (decoder != null)
...
//    }
    ...
    protected Rect calculateCacheWindow(Rect viewportRect) {
        ...
        if (DEBUG)
            Log.d(TAG, "new cache.originRect = " + calculatedCacheWindowRect.toShortString() + " size=" + size
                    .toString());
