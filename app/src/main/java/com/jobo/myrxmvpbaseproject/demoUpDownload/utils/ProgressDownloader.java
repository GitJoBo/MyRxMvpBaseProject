...
public class ProgressDownloader {
    ...
    public ProgressDownloader(String url, ProgressResponseBody.ProgressListener progressListener) {
        this.url = url;
        this.progressListener = progressListener;
        //在下载、暂停后的继续下载中可复用同一个client对象
        client = getProgressClient();
    ...
    private void save(Response response, long startsPoint) {
        LogUtils.d("aaaaaaaaa,response;"+response);
        ...
        try {
            if (destination == null) {
                ...
                if (!TextUtils.isEmpty(filename)){
                    ...
                    LogUtils.d("aaaaaaaaa,index="+index);
                    if (index!=-1){
                        filename = filename.substring(index);
                    }
                    LogUtils.d("aaaaaaaa,filename="+filename);
...
//                    filename = System.currentTimeMillis()+"";
                    filename = "qihoo360.mobilesafe_261.apk";
                }
