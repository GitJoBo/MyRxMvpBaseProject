package com.jobo.myrxmvpbaseproject.widget.view.NineImageView;
...
/**
 * 图片超过maxImageSize将在最后一张上显示剩余张数数字
* */
public class NineImageViewGroup extends ViewGroup {
    ...
    private int maxImageSize = 12;                   // 9 最大显示的图片数  Integer.MAX_VALUE
    ...
    public NineImageViewGroup(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NineImageViewGroup(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    private void init(Context context, AttributeSet attrs) {
        int WIDTH = (int) (DensityUtil.getScreenWidth(context) - DensityUtil.dip2px(context, 15 * 2));
    ...
    private void loadImage(ImageView imageView, ImageAttr attr, int count) {
        ...
        imageLoader.requestBuilder(url, requestOptions)
                ...
                .listener(new RequestListener<Drawable>() {
                    @Override
                    public boolean onLoadFailed(@Nullable GlideException e, Object model, com.bumptech.glide.request
                            .target.Target<Drawable> target, boolean isFirstResource) {
                        return false;
                    }
    ...
    private ImageView getImageView(int position) {
        ...
        if (imageView == null) {
            ...
            imageView.setOnClickListener(v -> mAdapter.onImageItemClick(getContext(), NineImageViewGroup.this, position, mAdapter.getImageAttrs()));
